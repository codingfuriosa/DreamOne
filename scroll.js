...
(() => {
  ...
  if (cinemaSection && cinemaMask) {
    ...
    // Vertical pill (portrait video) on iPad + mobile (<=1024); horizontal pill
    // (landscape video) only on laptop/desktop (>1024).
    ...
    gsap.timeline({
      scrollTrigger: {
        ...
        end: () => (window.innerWidth <= 1024 ? '+=260%' : '+=160%'),
        pin: true,
        pinSpacing: true,
        // Smoother scrub so the grow eases in from the pin point instead of
        // snapping. anticipatePin removed — it pre-shifted the layout right as
        // the pin engaged, which read as a jolt at the start of the zoom.
        scrub: 0.8,
        invalidateOnRefresh: true,
